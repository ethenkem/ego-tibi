...
It's composed of four small, independent services that communicate primarily through RabbitMQ events rather than calling each other directly — so each piece can be understood, deployed, and even replaced on its own, the way real production microservice systems are built.
...
3. A countdown starts, sized to the ticket's priority (5–60 minutes).
4. If an agent acknowledges the ticket in time, nothing else happens — ticket proceeds normally.
...
This is meant to run on a **server the IT team already controls** — not a developer's laptop, not a public cloud-facing endpoint. Realistic deployment targets:
...
It is **not**, in its current form, meant to be exposed directly to the public internet — there's no authentication layer yet (see Roadmap). It's built for internal, trusted-network use: an IT helpdesk that only people inside the company can reach, the same way most internal ticketing tools work today.
...
```
   ...
   IT agents) ───────────►│ (port 8080)    │
                         └──────┬───────┘
                                ...
                                ▼
                         ┌──────────────┐
                         │ Ticket Service │  (owns the database — source of truth)
                         └──────┬───────┘
                                  │ publishes
                                  ▼
                      ┌────────────────────────┐
...
| **Routing Service** | Applies category → team rules. Writes assignment back to Ticket Service. | None (stateless) |
...
That's the entire installation. Docker Compose pulls each service's image directly from Docker Hub (`docker.io/ethenkem/ego-tibi-ticket-service`, etc.), starts RabbitMQ and PostgreSQL alongside them, and wires up the network between all of them automatically.
...
Once submitted, you'll see the ticket get assigned to a team automatically, and — if it sits unacknowledged past its SLA window — escalate on its own.
...
| OTHER | General Support | IT Manager | — |
